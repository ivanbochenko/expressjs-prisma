...
const getFacebookEmail = async (code: string, verifier: string) => {
  ...
  link.searchParams.set("client_id", process.env.FACEBOOK_CLIENT_ID as string )
  link.searchParams.set("redirect_uri", expoUrl + process.env.EXPO_SLUG )
  link.searchParams.set("client_secret", process.env.FACEBOOK_CLIENT_SECRET as string )
